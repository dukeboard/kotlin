...
public class MultiDeclarationTranslator extends AbstractTranslator {
    ...
    @NotNull
    public static JsVars translateMultiDeclaration(@NotNull JetMultiDeclaration multiDeclaration,
            @NotNull JsName multiObjName, @NotNull JsExpression initializer, @NotNull TranslationContext context) {
        return new MultiDeclarationTranslator(multiDeclaration, multiObjName, initializer, context).translate();
    }

    @NotNull
    private final JetMultiDeclaration multiDeclaration;
    @NotNull
    private final JsName multiObjName;
    @NotNull
    private final JsExpression initializer;

    private MultiDeclarationTranslator(@NotNull JetMultiDeclaration multiDeclaration,
            @NotNull JsName multiObjName,
            @NotNull JsExpression initializer,
            @NotNull TranslationContext context
    ) {
        super(context);
        this.multiDeclaration = multiDeclaration;
        this.multiObjName = multiObjName;
        this.initializer = initializer;
    }

    private JsVars translate() {
        List<JsVars.JsVar> jsVars = new ArrayList<JsVars.JsVar>();
        jsVars.add(new JsVars.JsVar(multiObjName, initializer));

        JsNameRef multiObjNameRef = multiObjName.makeRef();
