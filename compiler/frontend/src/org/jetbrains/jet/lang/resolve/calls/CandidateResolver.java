...
public class CandidateResolver {
    ...
    private <D extends CallableDescriptor> void completeInferenceForArgument(
        ...
        JetType result = BindingContextUtils.updateRecordedType(
                type, expression, context.trace, isSafeCallExpression(expression, context.trace));
    ...
    private boolean isSafeCallExpression(@NotNull JetExpression expression, @NotNull BindingTrace trace) {
        if (expression instanceof JetSafeQualifiedExpression) {
            ...
            JetType type = trace.get(BindingContext.EXPRESSION_TYPE, safeQualifiedExpression.getSelectorExpression());
    ...
    private <D extends CallableDescriptor> void addConstraintForFunctionLiteral(
        ...
        if (hasExpectedReturnType) {
            ...
            CallCandidateResolutionContext<D> newContext =
                    context.replaceBindingTrace(errorInterceptingTrace).replaceExpectedType(expectedType).replaceDataFlowInfo(dataFlowInfoForArgument);
            ...
            temporaryToResolveFunctionLiteral.cache.commit();
        }
