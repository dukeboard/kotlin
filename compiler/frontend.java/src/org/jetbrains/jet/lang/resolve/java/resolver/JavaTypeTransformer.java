...
public class JavaTypeTransformer {
    ...
    private JetType transformClassType(
        ...
        if (classData == null) {
            classData = classResolver.resolveClass(fqName, DescriptorSearchRule.INCLUDE_KOTLIN);
        }
        if (classData == null) {
            return null;
        }
        ...
        if (isRaw(classifierType, !parameters.isEmpty())) {
            ...
            Collection<JavaType> javaTypeArguments = classifierType.getArguments();
    ...
    private static boolean isRaw(@NotNull JavaClassifierType classifierType, boolean argumentsExpected) {
        ...
        return classifierType.isRaw() || argumentsExpected && classifierType.getArguments().isEmpty();
    }
}
